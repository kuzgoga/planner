...
<template>
  <ClientOnly>
    ...
    <a-flex v-if="show" :align="'center'" :justify="'center'" class="w-full h-full z-50 absolute">
      <a-form class="border border-gray-300 rounded-2xl w-[600px] h-[300px] bg-white" style="padding: 10px">
        <a-flex :justify="'space-between'" class="w-full h-full" vertical>
          ...
          <VueDatePicker placeholder="Дата кончала" time-picker-inline v-model="data.end" :min-date="data.start">
          </VueDatePicker>
          <a-input placeholder="Адрес" v-model:value="data.location"></a-input>
          <a-select :filter-option v-model:value="<any[]>data.participants" mode="multiple" placeholder="Участники"
            :options="users"></a-select>
